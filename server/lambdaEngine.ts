...
export const SAMPLE_FUNCTIONS: LambdaFunctionDetails[] = [
  {
    functionArn: 'arn:aws:lambda:us-east-1:123456789012:function:WorkerProcessor',
    functionName: 'WorkerProcessor',
    runtime: 'nodejs20.x',
    memorySize: 1024,
    timeout: 30,
  ...
  {
    functionArn: 'arn:aws:lambda:us-east-1:123456789012:function:AuthWorkerService',
    functionName: 'AuthWorkerService',
    runtime: 'python3.11',
    memorySize: 512,
    timeout: 15,
  ...
  {
    functionArn: 'arn:aws:lambda:us-east-1:123456789012:function:DocGenerator',
  ...
  {
    functionArn: 'arn:aws:lambda:us-east-1:123456789012:function:NotificationDispatcher',
  ...
  {
    functionArn: 'arn:aws:lambda:us-east-1:123456789012:function:BatchSyncJob',
    functionName: 'BatchSyncJob',
    runtime: 'python3.8',
    memorySize: 1536,
    timeout: 300,
    handler: 'sync.main',
...
export async function getFunctionHealth(
  ...
  const isWarning = functionName === 'DocGenerator';
  const isCritical = functionName === 'BatchSyncJob';
...
export async function getPerformanceMetrics(
  ...
  const baseInv = functionName === 'WorkerProcessor' ? 450 : functionName === 'DocGenerator' ? 120 : 250;
  const baseDur = functionName === 'DocGenerator' ? 4200 : functionName === 'WorkerProcessor' ? 380 : 120;
  ...
  for (let i = points; i >= 0; i--) {
    ...
    const thr = functionName === 'DocGenerator' && i === 4 ? 12 : 0;
  ...
  return [
    {
      ...
      stackTrace: `at com.example.service.WorkerService.process(PaymentService.java:142)\nat com.example.worker.Handler.handleRequest(Handler.java:45)\nat lambdainternal.EventHandlerLoader$2.call(EventHandlerLoader.java:902)`,
...
export async function getColdStartDiagnostic(
  ...
  const isJava = functionName === 'DocGenerator';
  return {
    functionName,
...
export async function getCostAnalysis(
  ...
  const isHighCost = functionName === 'DocGenerator';
  return {
    functionName,
...
export async function getMemoryRecommendation(
  ...
  if (functionName === 'WorkerProcessor') {
    return {
      allocatedMb: 1024,
      usedMb: 195,
      peakMb: 260,
  ...
  } else if (functionName === 'DocGenerator') {
    return {
      allocatedMb: 2048,
      usedMb: 1820,
      peakMb: 1980,
...
export async function getTimeoutDiagnostic(
  ...
  const isNear = functionName === 'DocGenerator';
  return {
...
export async function getSecurityPosture(
  ...
  const isInvoice = functionName === 'DocGenerator';
  const isLegacy = functionName === 'BatchSyncJob';
...
export async function getDependencyGraph(
  ...
  return {
    nodes: [
      ...
      { id: 'fn_main', name: functionName, type: 'Lambda', status: functionName === 'DocGenerator' ? 'Warning' : 'Healthy' },
...
export async function getAIInsights(
  ...
  if (functionName === 'DocGenerator') {
    return {
      functionName,
      issueTitle: 'Duration & Latency Spike (+37%)',
      summary: 'DocGenerator execution duration increased by 37% over the past 24 hours, correlating with memory pressure and recent v21 deployment.',
      possibleCauses: [
...
export async function getLiveCloudWatchMetrics(
  ...
  // Synthetic fallback
  const points = 24;
  const baseInv = functionName === 'WorkerProcessor' ? 450 : 180;
  const baseDur = functionName === 'DocGenerator' ? 4200 : 380;
  const now = new Date();
...
export async function getLambdaLogStream(
  ...
  const syntheticLines: LambdaLogLine[] = [
    { timestamp: new Date(now - 3200).toISOString(), message: `START RequestId: ${reqIds[0]} Version: $LATEST`, level: 'START' as const, requestId: reqIds[0] },
    { timestamp: new Date(now - 3100).toISOString(), message: 'INIT_START Runtime Version: nodejs:20.v23', level: 'INIT' as const, isColdStart: true, initDurationMs: 412 },
    { timestamp: new Date(now - 2800).toISOString(), message: '[INFO] Initializing DB connection pool', level: 'INFO' as const, requestId: reqIds[0] },
    { timestamp: new Date(now - 2600).toISOString(), message: '[INFO] Processing payment for customer_id=cust_7829', level: 'INFO' as const, requestId: reqIds[0] },
    { timestamp: new Date(now - 2400).toISOString(), message: `REPORT RequestId: ${reqIds[0]} Duration: 382.41 ms Billed Duration: 383 ms Memory Size: 1024 MB Max Memory Used: 238 MB Init Duration: 412.10 ms`, level: 'REPORT' as const, requestId: reqIds[0], durationMs: 382, memoryMb: 238, initDurationMs: 412, isColdStart: true },
    { timestamp: new Date(now - 2200).toISOString(), message: `START RequestId: ${reqIds[1]} Version: $LATEST`, level: 'START' as const, requestId: reqIds[1] },
    { timestamp: new Date(now - 2000).toISOString(), message: '[ERROR] Connection pool exhausted: Timeout waiting for idle connection', level: 'ERROR' as const, requestId: reqIds[1] },
    { timestamp: new Date(now - 1900).toISOString(), message: `REPORT RequestId: ${reqIds[1]} Duration: 30001.00 ms Billed Duration: 30001 ms Memory Size: 1024 MB Max Memory Used: 610 MB`, level: 'REPORT' as const, requestId: reqIds[1], durationMs: 30001, memoryMb: 610, isColdStart: false },
    { timestamp: new Date(now - 1600).toISOString(), message: `START RequestId: ${reqIds[2]} Version: $LATEST`, level: 'START' as const, requestId: reqIds[2] },
    { timestamp: new Date(now - 1400).toISOString(), message: '[INFO] Processing payment for customer_id=cust_4421', level: 'INFO' as const, requestId: reqIds[2] },
    { timestamp: new Date(now - 1200).toISOString(), message: '[INFO] Payment processed successfully. charge_id=ch_9234', level: 'INFO' as const, requestId: reqIds[2] },
    { timestamp: new Date(now - 1000).toISOString(), message: `REPORT RequestId: ${reqIds[2]} Duration: 245.11 ms Billed Duration: 246 ms Memory Size: 1024 MB Max Memory Used: 228 MB`, level: 'REPORT' as const, requestId: reqIds[2], durationMs: 245, memoryMb: 228, isColdStart: false },
