...
export interface SloMetrics {
  ...
  allowedDownSec: number;
  consumedDownSec: number;
  remainingBudgetSec: number;
  remainingBudgetPercent: number;
  burnRateStatus: 'NORMAL' | 'MODERATE_BURN' | 'CRITICAL_BURN';
  incidentCount: number;
}

export async function calculateTargetSlo(targetId: string, targetSloPercent: number = 99.9): Promise<SloMetrics> {
  const windowDays = 30;
  ...
  try {
    ...
    const { rows } = await query(
      `SELECT COALESCE(SUM(CASE WHEN "isResolved" THEN "durationSec" ELSE GREATEST(1, EXTRACT(EPOCH FROM (NOW() - "startedAt"))::int) END), 0) AS "totalDownSec", COUNT(*) AS "totalCount"
       FROM url_incidents
       WHERE "targetId" = $1 AND "startedAt" >= $2`,
      [targetId, cutOff]
    );
  ...
  let burnRateStatus: 'NORMAL' | 'MODERATE_BURN' | 'CRITICAL_BURN' = 'NORMAL';
