...
export interface RouteCostSummary {
  ...
  method: string;
  totalCalls: number;
  apiGatewayCostUsd: number;
  lambdaExecCostUsd: number;
  totalCostUsd: number;
 ...
 */
export async function calculateRouteFinOpsCosts(apiId: string, stage: string, protocol: 'REST' | 'HTTP' = 'REST'): Promise<RouteCostSummary[]> {
  const routeCosts: RouteCostSummary[] = [];
...
export interface MemoryRightSizingRecommendation {
  functionName: string;
  allocatedMemoryMb: number;
  peakMemoryUsedMb: number;
  recommendedMemoryMb: number;
  overProvisionedRatio: number;
  monthlyCurrentCostUsd: number;
  monthlyOptimizedCostUsd: number;
  monthlySavingsUsd: number;
  recommendationLevel: 'HIGH_SAVINGS' | 'MODERATE_SAVINGS' | 'OPTIMAL';
}
...
export function calculateLambdaMemoryRightSizing(functions: any[]): MemoryRightSizingRecommendation[] {
  return functions.map((fn) => {
    ...
    const level: 'HIGH_SAVINGS' | 'MODERATE_SAVINGS' | 'OPTIMAL' =
      savings > 15 ? 'HIGH_SAVINGS' : savings > 5 ? 'MODERATE_SAVINGS' : 'OPTIMAL';
