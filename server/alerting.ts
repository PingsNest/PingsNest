...
export interface AlertRule {
  ...
  threshold: number;
  intervalMinutes: number;      // minimum minutes between firings
  webhookUrl: string;
  channel: ChannelType;
  enabled: boolean;
  createdAt: string;
...
const lastFiredAt = new Map<string, number>(); // ruleId → ms timestamp
...
export async function evaluateAlerts(
  ...
  try {
    const { rows } = await query<AlertRule>(
      `SELECT * FROM alert_rules WHERE ("apiId"=$1 OR "apiId"='*') AND (stage=$2 OR stage='*') AND enabled=true`,
      [apiId, stage]
    );
    rules = rows;
  } catch {
  ...
  for (const rule of rules) {
    ...
    try {
      await query(
        ...
        [rule.id, rule.name, apiId, stage, rule.metric, value, rule.threshold]
      );
    } catch { /* non-fatal */ }
...
// ─── Webhook delivery ─────────────────────────────────────────────────────────
export async function fireWebhook(rule: AlertRule, value: number): Promise<void> {
...
function buildWebhookBody(rule: AlertRule, value: number): object {
...
export async function fireUrlTargetWebhook(
  webhookUrl: string,
  channel: ChannelType,
  target: { name: string; url: string; lastStatusCode?: number; lastStatusText?: string; certExpDays?: number },
...
// ─── SLO Multi-Window Burn Rate Alerting (Google SRE Standard) ───────────────
export interface SloBurnRateResult {
  sloName: string;
  targetPercent: number;
  burnRate1h: number;
  burnRate6h: number;
  isAlert1hFired: boolean;
  isAlert6hFired: boolean;
}

export function calculateSloBurnRate(
  targetPercent: number,
  errorCount1h: number,
  totalCount1h: number,
  errorCount6h: number,
  totalCount6h: number
): { burnRate1h: number; burnRate6h: number; alert1h: boolean; alert6h: boolean } {
  const allowedErrorRate = (100 - targetPercent) / 100;
  if (allowedErrorRate <= 0) return { burnRate1h: 0, burnRate6h: 0, alert1h: false, alert6h: false };
