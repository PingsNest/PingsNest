...
import { ApiGatewayV2Client, GetApisCommand, GetRoutesCommand, GetIntegrationsCommand, GetStageCommand as GetStageV2Command, UpdateStageCommand as UpdateStageV2Command } from '@aws-sdk/client-apigatewayv2';
...
import { cacheGet, cacheSet, cacheDel, getRedisStats } from './cache.js';
...
app.get('/api/finops/costs', async (req, res) => {
  ...
  if (!apiId || !stage) return res.status(400).json({ error: 'Missing apiId or stage' });
  const routeCosts = await calculateRouteFinOpsCosts(String(apiId), String(stage), (protocol as any) || 'REST');
  res.json({ routeCosts });
});
...
async function saveTarget(t: UrlTarget): Promise<void> {
  await query(
    `INSERT INTO targets (id, name, url, interval, method, headers, body, "bodyEncoding", status, timeout, retries, "retryInterval", "groupName", "certExpiryDate", "certExpDays", "lastCheck", "lastStatusCode", "lastStatusText", "lastLatency", "isUp", "recentPings", steps)
...
async function pingTarget(target: UrlTarget): Promise<UrlTarget> {
  ...
  try {
    ...
    if (!isUp && !hasOpenIncident && !isMaintenanceMuted) {
      ...
      } catch (alertErr) { console.error('[URL Outage Alert Error]:', alertErr); }
      ...
      } catch (alertErr) { console.error('[URL Recovery Alert Error]:', alertErr); }
    }
  } catch (incErr) {
...
app.post('/api/url-monitor/targets', requireAuth, async (req, res) => {
  ...
  let newTarget: UrlTarget = { id: Math.random().toString(36).substring(2, 9), name, url, interval: Number(interval) || 60, method: method || 'GET', headers, body, bodyEncoding: bodyEncoding || 'JSON', status: 'active', timeout: typeof timeout !== 'undefined' ? Number(timeout) : 48, retries: typeof retries !== 'undefined' ? Number(retries) : 0, retryInterval: typeof retryInterval !== 'undefined' ? Number(retryInterval) : 60, group: group || '' };
...
app.put('/api/url-monitor/targets/:id', requireAuth, async (req, res) => {
  ...
  const updatedTarget: UrlTarget = { ...targets[idx], name: name || targets[idx].name, url: url || targets[idx].url, interval: typeof interval !== 'undefined' ? Number(interval) : targets[idx].interval, method: method || targets[idx].method, headers: typeof headers !== 'undefined' ? headers : targets[idx].headers, body: typeof body !== 'undefined' ? body : targets[idx].body, bodyEncoding: typeof bodyEncoding !== 'undefined' ? bodyEncoding : targets[idx].bodyEncoding, timeout: typeof timeout !== 'undefined' ? Number(timeout) : targets[idx].timeout, retries: typeof retries !== 'undefined' ? Number(retries) : targets[idx].retries, retryInterval: typeof retryInterval !== 'undefined' ? Number(retryInterval) : targets[idx].retryInterval, group: typeof group !== 'undefined' ? group : targets[idx].group };
...
app.post('/api/playbooks', async (req, res) => {
  ...
  try {
    ...
    await query(
      `INSERT INTO remediation_playbooks (id, name, description, enabled, "targetType", "targetId", condition, threshold, action, "actionPayload", "cooldownMinutes", "createdAt")
       ...
       ON CONFLICT (id) DO UPDATE SET
         ...
         action=EXCLUDED.action, "actionPayload"=EXCLUDED."actionPayload", "cooldownMinutes"=EXCLUDED."cooldownMinutes"`,
      [pbId, name, description || null, targetType, targetId, condition, threshold, action, actionPayload || null, cooldownMinutes]
    );
