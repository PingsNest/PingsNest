...
export interface TenantTierQuota {
  tier: 'free' | 'pro' | 'enterprise';
  monthlyLimit: number;
}

const TIER_QUOTAS: Record<string, TenantTierQuota> = {
...
export function tenantQuotaMiddleware(req: Request, res: Response, next: NextFunction): void {
  ...
  const tenantTier = ((req.headers['x-tenant-tier'] as string) || 'free').toLowerCase();
  ...
  query(
    `SELECT COUNT(*) AS count
     FROM gateway_logs
     WHERE "tenantId" = $1 AND "fullTime" >= date_trunc('month', NOW())`,
    [tenantId]
  )
    .then(({ rows }) => {
