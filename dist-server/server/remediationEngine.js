 ...
 */
export async function evaluateAndExecutePlaybooks(ctx) {
    try {
        const { rows: playbooks } = await query(`SELECT * FROM remediation_playbooks
       WHERE enabled = true AND ("targetId" = $1 OR "targetId" = '*')`, [ctx.targetId]);
        if (playbooks.length === 0)
            return;
        for (const pb of playbooks) {
            // Check condition match
            const triggerMatched = pb.condition === '*' ||
            ...
            const { rows: hourlyCount } = await query(`SELECT COUNT(*) AS count FROM playbook_history
         WHERE "playbookId" = $1 AND status = 'SUCCESS' AND "executedAt" >= NOW() - INTERVAL '1 hour'`, [pb.id]);
            ...
            await recordPlaybookExecution(pb.id, pb.name, ctx.triggerType, pb.action, execResult.success ? 'SUCCESS' : 'FAILED', execResult.details);
            broadcastAlert({
...
async function executeSelfHealingAction(action, payloadStr, ctx) {
    try {
        if (action === 'cache_flush' || action === 'flush_cache') {
            ...
            if (!payloadStr)
                return { success: false, details: 'Missing webhook payload/URL configuration.' };
...
async function recordPlaybookExecution(playbookId, playbookName, trigger, action, status, details) {
    try {
       ...
       VALUES ($1, $2, $3, $4, $5, $6, $7, NOW())`, [histId, playbookId, playbookName, trigger, action, status, details]);
    }
    catch (err) {
