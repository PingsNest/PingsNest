...
export async function saveSESConfig(cfg) {
    await query(`
...
export async function sendEmailViaSMTP(subject, htmlBody, cfgOverride) {
...
export async function loadWebhookChannelsConfig() {
    try {
...
export async function saveWebhookChannelsConfig(cfg) {
    await query(`
...
let inMemoryGenericAlertRules = null;
export async function loadGenericAlertRules() {
...
export async function saveGenericAlertRules(r) {
...
let inMemoryAlertHistory = [];
let silencedAlerts = new Map();
let acknowledgedAlerts = new Set();
export function silenceAlert(targetOrRuleId, durationMinutes) {
    const expiresAt = Date.now() + durationMinutes * 60 * 1000;
    silencedAlerts.set(targetOrRuleId, expiresAt);
    return { success: true, expiresAt: new Date(expiresAt).toISOString() };
}
export function isAlertSilenced(targetOrRuleId) {
...
export function isAlertAcknowledged(alertId) {
...
export async function logAlertDispatch(item) {
    ...
    if (inMemoryAlertHistory.length > 200)
        inMemoryAlertHistory.pop();
    try {
        await query(`
...
export async function sendEmailViaSES(subject, htmlBody, cfgOverride) {
...
export async function loadAlertDestinations() {
    try {
...
export async function dispatchAlertNotification(event, target, extraDetails) {
...
export function buildHTMLNotificationTemplate(alert) {
...
export function buildSlackBlockKitTemplate(alert) {
    const isCritical = alert.severity === 'critical';
...
export function buildMSTeamsAdaptiveCardTemplate(alert) {
    const isCritical = alert.severity === 'critical';
    const statusTitle = isCritical ? 'CRITICAL API GATEWAY ALERT' : 'WARNING API GATEWAY ALERT';
    const fullDetailedText = [
...
export function buildDiscordEmbedTemplate(alert) {
    const isCritical = alert.severity === 'critical';
    const statusTitle = isCritical ? 'CRITICAL API GATEWAY ALERT' : 'WARNING API GATEWAY ALERT';
    const fullDetailedText = [
...
export function buildPagerDutyPayloadTemplate(alert, routingKey) {
    const isCritical = alert.severity === 'critical';
    return {
        payload: {
...
export async function dispatchGatewayFleetAlert(alert) {
...
export async function dispatchUrlMonitorAlert(payload) {
...
const flappingHistory = new Map();
export function recordAlertStateTransition(ruleOrTargetId) {
    const now = Date.now();
...
export function getFlappingAlerts() {
    const result = [];
    const now = Date.now();
...
let inMemoryEscalationPolicies = [
    {
        id: 'policy-default',
...
export function saveEscalationPolicy(policy) {
...
const activeSlaTracks = new Map();
export function trackAlertForEscalation(alertId, ruleName, policyId = 'policy-default') {
...
export function checkAndTriggerEscalations() {
    const escalated = [];
    const now = Date.now();
...
const ruleRemediationMap = new Map(); // ruleId -> playbookId
export function associateRuleWithRemediation(ruleId, playbookId) {
    ruleRemediationMap.set(ruleId, playbookId);
}
export function getRemediationPlaybookForRule(ruleId) {
