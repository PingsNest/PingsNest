 ...
 */
export function evaluateSyntheticAssertions(statusCode, headers, bodyText, assertions) {
    const results = [];
    let allPassed = true;
        ...
        try {
            if (rule.type === 'status_code') {
                ...
                if (rule.operator === 'equals')
                    passed = String(val) === String(rule.expectedValue);
                else if (rule.operator === 'contains')
                    passed = String(val).toLowerCase().includes(rule.expectedValue.toLowerCase());
